...
def main(args):
    ...
    Args:
       ...
       * **num_modes** (int): Number of modes to compute (defaults=1)
       ...
       * **plot_mode** (boolean): Save the mode image and data to a separate file (default=None)
    ...
    k = ms.find_k(parity, freq, plot_mode_number, num_modes, kdir, tol, kmag_guess, kmag_min, kmag_max)
    vg = ms.compute_group_velocities()

    print(k)
    print(vg)

    print("num_bands="+str(num_modes))
    print("plot_mode_number="+str(plot_mode_number))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-plot_mode', type=str2bool, nargs='?', const=True, default=True, help='Save the mode image and data to a separate file (default=True)')

...
class MMI1x2(gdspy.Cell):
    ...
    def build_cell(self):
        ...
        if self.direction=="EAST":
            self.output_port_top = (self.port[0]+totlength, self.port[1]+self.wg_sep/2.0)
            self.output_port_bot = (self.port[0]+totlength, self.port[1]-self.wg_sep/2.0)
        elif self.direction=="NORTH":
            self.output_port_top = (self.port[0]-self.wg_sep/2.0, self.port[1]+totlength)
            self.output_port_bot = (self.port[0]+self.wg_sep/2.0, self.port[1]+totlength)
            angle=np.pi/2.0

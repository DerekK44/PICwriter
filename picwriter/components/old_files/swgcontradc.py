...
class SWGContraDirectionalCoupler(gdspy.Cell):
    """ Standard SWG Contra-Directional Coupler Cell class (subclass of gdspy.Cell).  For more details on the principles and operation behind this type of component, please see https://doi.org/10.1364/OE.21.003633.
        This component uses a discontinuous sub-wavelength grating (SWG) waveguide in the coupler region.  Basically two DBR's components close together.
        ...
        Args:
           ...
           * **w_phc_top** (float): Width of the thin section of the top waveguide.  w_phc_top = 0 corresponds to disconnected periodic blocks.
        ...
        Keyword Args:
           ...
           * **bot_angle** (float): Angle in radians (between 0 and pi/2) at which the *bottom* waveguide bends towards the coupling region.  Default=pi/6.
           ...
           * **apodization_curv** (float): If `apodization_top`=`True`, then this parameter sets the curvature for the Gaussian apodization.  Defaults to (1/length)**2.
    ...
    """
    def __init__(self, wgt, length, gap, period, dc, taper_length, w_phc_top, w_phc_bot, top_angle=np.pi/6.0, bot_angle=np.pi/6.0, width_top=None, width_bot=None, input_bot=False, apodization_top=False, apodization_far_dist=1.0, apodization_curv=None, fins=False, fin_size=(0.2, 0.05), contradc_wgt=None, port=(0,0), direction='EAST'):
        ...
        if bot_angle > np.pi/2.0 or bot_angle < 0:
            raise ValueError("Warning! Improper bot_angle specified ("+str(bot_angle)+").  Must be between 0 and pi/2.0.")
        self.top_angle = top_angle
        self.bot_angle = bot_angle
    ...
    def build_cell(self):
        ...
        max_angle = max([self.top_angle, self.bot_angle])
        dlx = abs(self.wgt.bend_radius*np.tan((max_angle)/2.0))
        angle_x_dist = 2*self.wgt.bend_radius*np.sin(max_angle)
        if self.top_angle > self.bot_angle:
            top_br = self.wgt.bend_radius
            if self.bot_angle < 1E-6:
                bot_br = None
            else:
                bot_br = (0.5*angle_x_dist)/np.sin(self.bot_angle)
        else:
            bot_br = self.wgt.bend_radius
            if self.top_angle < 1E-6:
                top_br = None
            else:
                top_br = (0.5*angle_x_dist)/np.sin(self.top_angle)
        angle_y_dist_top = 0 if top_br==None else 2*top_br*(1-np.cos(self.top_angle))
        angle_y_dist_bot = 0 if bot_br==None else 2*bot_br*(1-np.cos(self.bot_angle))
        distx = 2*angle_x_dist + self.length
        disty = (abs(angle_y_dist_top) + abs(angle_y_dist_bot) + self.gap + (self.width_top+self.width_bot)/2.0)*self.parity

        if self.parity==1:
            shift = 0
        elif self.parity==-1:
            shift = (angle_y_dist_top + angle_y_dist_bot + self.gap + (self.width_top+self.width_bot)/2.0)
        ...
        def gaussian_top(t): #Gaussian path only used for apodized coupler gaps, t varies from 0 to 1
            x = x01 + angle_x_dist + self.taper_length + t*(self.length-2*self.taper_length)
        ...
        if self.w_phc_top > 1E-6: #Connected TOP SWG waveguides
            wg_top = gdspy.Path(self.wgt.wg_width, (x01, y01))
            if top_br != None: #Below for curved top waveguide
                wg_top.turn(top_br, -self.top_angle, number_of_points=0.1, **self.wg_spec)
                wg_top.turn(top_br, self.top_angle, number_of_points=0.1, final_width=self.width_top, **self.wg_spec)
                wg_top.segment(self.taper_length, final_width=self.w_phc_top, **self.wg_spec)
                if self.apodization_top:
                    wg_apod = gdspy.Path(self.w_phc_top, (0, 0))
                    wg_apod.direction='+x'
                    ...
                    wg_top.segment(self.length-2*self.taper_length, **self.wg_spec)
                wg_top.segment(self.taper_length, final_width=self.width_top, **self.wg_spec)
                wg_top.turn(top_br, self.top_angle, number_of_points=0.1, final_width=self.wgt.wg_width, **self.wg_spec)
                wg_top.turn(top_br, -self.top_angle, number_of_points=0.1, **self.wg_spec)
            else: #Below for straight top waveguide (no bend)
                wg_top.segment(angle_x_dist-self.taper_length, final_width=self.width_top, **self.wg_spec)
                wg_top.segment(self.taper_length, final_width=self.w_phc_top, **self.wg_spec)
                wg_top.segment(self.length, **self.wg_spec)
                wg_top.segment(self.taper_length, final_width=self.width_top, **self.wg_spec)
                wg_top.segment(angle_x_dist-self.taper_length, final_width=self.wgt.wg_width, **self.wg_spec)
        else: # Unconnected TOP SWG waveguides (2 paths)
            wg_top = gdspy.Path(self.wgt.wg_width, (x01, y01))
            if top_br != None: #Below for curved top waveguide
                wg_top.turn(top_br, -self.top_angle, number_of_points=0.1, **self.wg_spec)
                wg_top.turn(top_br, self.top_angle, number_of_points=0.1, final_width=self.width_top, **self.wg_spec)
                wg_top.segment(self.taper_length, final_width=0.0, **self.wg_spec)

                wg_top2 = gdspy.Path(self.wgt.wg_width, (x01+distx, y01))
                wg_top2.direction='-x'
                wg_top2.turn(top_br, self.top_angle, number_of_points=0.1, **self.wg_spec)
                wg_top2.turn(top_br, -self.top_angle, number_of_points=0.1, final_width=self.width_top, **self.wg_spec)
                wg_top2.segment(self.taper_length, final_width=0.0, **self.wg_spec)
            else: #Below for straight top waveguide (no bend)
                wg_top.segment(angle_x_dist-self.taper_length, final_width=self.width_top, **self.wg_spec)
                wg_top.segment(self.taper_length, final_width=0.0, **self.wg_spec)
                wg_top2 = gdspy.Path(self.wgt.wg_width, (x01+distx, y01))
                wg_top2.direction='-x'
                wg_top2.segment(angle_x_dist-self.taper_length, final_width=self.width_top, **self.wg_spec)
                wg_top2.segment(self.taper_length, final_width=0.0, **self.wg_spec)

        if top_br != None: # For curved top waveguide cladding
            wg_top_clad = gdspy.Path(2*self.wgt.clad_width+self.wgt.wg_width, (x01, y01))
            wg_top_clad.turn(top_br, -self.top_angle, number_of_points=0.1, **self.clad_spec)
            wg_top_clad.turn(top_br, self.top_angle, number_of_points=0.1, final_width=self.width_top+2*self.wgt.clad_width, **self.clad_spec)
            wg_top_clad.segment(self.length, **self.clad_spec)
            wg_top_clad.turn(top_br, self.top_angle, number_of_points=0.1, final_width=self.wgt.wg_width+2*self.wgt.clad_width, **self.clad_spec)
            wg_top_clad.turn(top_br, -self.top_angle, number_of_points=0.1, **self.clad_spec)
        else: # For straight top waveguide cladding
            wg_top_clad = gdspy.Path(2*self.wgt.clad_width+self.wgt.wg_width, (x01, y01))
            wg_top_clad.segment(angle_x_dist, final_width=self.width_top+2*self.wgt.clad_width, **self.clad_spec)
            wg_top_clad.segment(self.length, **self.clad_spec)
            wg_top_clad.segment(angle_x_dist, final_width=self.wgt.wg_width+2*self.wgt.clad_width, **self.clad_spec)

        x02, y02 = self.port[0], self.port[1] - (angle_y_dist_top + angle_y_dist_bot + self.gap + (self.width_top+self.width_bot)/2.0) + shift

        """ Add the bottom waveguide """
        if self.w_phc_bot > 1E-6:
            wg_bot = gdspy.Path(self.wgt.wg_width, (x02, y02))
            if bot_br != None: #Below for curved bottom waveguide
                wg_bot.turn(bot_br, self.bot_angle, number_of_points=0.1, **self.wg_spec)
                wg_bot.turn(bot_br, -self.bot_angle, number_of_points=0.1, final_width=self.width_bot, **self.wg_spec)
                wg_bot.segment(self.taper_length, final_width=self.w_phc_bot, **self.wg_spec)
                wg_bot.segment(self.length-2*self.taper_length, **self.wg_spec)
                wg_bot.segment(self.taper_length, final_width=self.width_bot, **self.wg_spec)
                wg_bot.turn(bot_br, -self.bot_angle, number_of_points=0.1, final_width=self.wgt.wg_width, **self.wg_spec)
                wg_bot.turn(bot_br, self.bot_angle, number_of_points=0.1, **self.wg_spec)
            else: #Below for straight bottom waveguide
                wg_bot.segment(angle_x_dist-self.taper_length, final_width=self.width_bot, **self.wg_spec)
                wg_bot.segment(self.taper_length, final_width=self.w_phc_bot, **self.wg_spec)
                wg_bot.segment(self.length, **self.wg_spec)
                wg_bot.segment(self.taper_length, final_width=self.width_bot, **self.wg_spec)
                wg_bot.segment(angle_x_dist-self.taper_length, final_width=self.wgt.wg_width, **self.wg_spec)
        else: # Unconnected bottom SWG waveguides (2 paths)
            wg_bot = gdspy.Path(self.wgt.wg_width, (x02, y02))
            if bot_br != None: #Below for curved bottom waveguide (SWG disconnected)
                wg_bot.turn(bot_br, self.bot_angle, number_of_points=0.1, **self.wg_spec)
                wg_bot.turn(bot_br, -self.bot_angle, number_of_points=0.1, final_width=self.width_bot, **self.wg_spec)
                ...
                wg_bot2.direction='-x'
                wg_bot2.turn(bot_br, -self.bot_angle, number_of_points=0.1, **self.wg_spec)
                wg_bot2.turn(bot_br, self.bot_angle, number_of_points=0.1, final_width=self.width_bot, **self.wg_spec)
                wg_bot2.segment(self.taper_length, final_width=0.0, **self.wg_spec)
            else: #Below for straight bottom waveguide (SWG disconnected)
                wg_bot.segment(angle_x_dist-self.taper_length, final_width=self.width_bot, **self.wg_spec)
                wg_bot.segment(self.taper_length, final_width=0.0, **self.wg_spec)
                wg_bot2 = gdspy.Path(self.wgt.wg_width, (x02+distx, y02))
                wg_bot2.direction='-x'
                wg_bot2.segment(angle_x_dist-self.taper_length, final_width=self.width_bot, **self.wg_spec)
                wg_bot2.segment(self.taper_length, final_width=0.0, **self.wg_spec)

        if bot_br != None: # For curved bottom waveguide cladding
            wg_bot_clad = gdspy.Path(2*self.wgt.clad_width+self.wgt.wg_width, (x02, y02))
            wg_bot_clad.turn(bot_br, self.bot_angle, number_of_points=0.1, **self.clad_spec)
            wg_bot_clad.turn(bot_br, -self.bot_angle, number_of_points=0.1, final_width=2*self.wgt.clad_width+self.width_bot, **self.clad_spec)
            wg_bot_clad.segment(self.length, **self.clad_spec)
            wg_bot_clad.turn(bot_br, -self.bot_angle, number_of_points=0.1, final_width=2*self.wgt.clad_width+self.wgt.wg_width, **self.clad_spec)
            wg_bot_clad.turn(bot_br, self.bot_angle, number_of_points=0.1, **self.clad_spec)
        else: # For straight bottom waveguide cladding
            wg_bot_clad = gdspy.Path(2*self.wgt.clad_width+self.wgt.wg_width, (x02, y02))
        ...
        if (bot_br==None and abs(self.w_phc_top-self.width_top)<=1E-6) or (top_br==None and abs(self.w_phc_bot-self.width_bot)<=1E-6):
            num_blocks = (self.length+2*self.taper_length)//self.period
        else:
        ...
        for i in range(int(num_blocks)):
            ...
            if abs(self.w_phc_top-self.width_top)>1E-6:
                block_list.append(gdspy.Rectangle((x, y0+self.gap/2.0), (x+blockx, y0+self.gap/2.0+self.width_top), **self.wg_spec))
        ...
        if self.w_phc_top <= 1E-6: # Unconnected top SWG waveguides (2 paths)
...
if __name__ == "__main__":
    ...
    cdc = SWGContraDirectionalCoupler(wgt, length=40.0, gap=0.2, period=0.5, dc=0.5, taper_length=5.0, w_phc_top=2.0, w_phc_bot=0.0,
                                        apodization_top=True, top_angle=np.pi/8, bot_angle=0, width_top=2.0, width_bot=1.0, input_bot=False, contradc_wgt=contradc_wgt, fins=True, **wg1.portlist["output"])
    tk.add(top, cdc)

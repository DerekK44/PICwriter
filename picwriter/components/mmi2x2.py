...
class MMI2x2(gdspy.Cell):
    ...
    def build_cell(self):
        ...
        # Bottom input
        path2 = gdspy.Path(self.wgt.wg_width, (self.port[0], self.port[1]-self.wg_sep))
        path2.segment(self.taper_length, direction='+x', final_width=self.taper_width, **self.wg_spec)
        # MMI body
        path3 = gdspy.Path(self.width, (self.port[0]+self.taper_length, self.port[1]-self.wg_sep/2.0))
        path3.segment(self.length, direction='+x', **self.wg_spec)
        # Top output
        path4 = gdspy.Path(self.wgt.wg_width, (self.port[0]+2*self.taper_length+self.length, self.port[1]))
        path4.segment(self.taper_length, direction='-x',final_width=self.taper_width, **self.wg_spec)
        # Bottom output
        path5 = gdspy.Path(self.wgt.wg_width, (self.port[0]+2*self.taper_length+self.length, self.port[1]-self.wg_sep))
        path5.segment(self.taper_length, direction='-x', final_width=self.taper_width, **self.wg_spec)
        ...
        clad_pts = [(self.port[0], self.port[1]+self.wgt.wg_width/2.0+self.wgt.clad_width),
                    ...
                    (self.port[0]+2*self.taper_length+self.length, self.port[1]-self.wg_sep/2.0-self.wgt.wg_width/2.0-self.wgt.clad_width-self.wg_sep),
                    (self.port[0]+self.taper_length+self.length, self.port[1]-self.width/2.0-self.wgt.clad_width-self.wg_sep),
                    (self.port[0]+self.taper_length, self.port[1]-self.width/2.0-self.wgt.clad_width-self.wg_sep),
                    (self.port[0], self.port[1]-self.wgt.wg_width/2.0-self.wgt.clad_width-self.wg_sep)]
        clad = gdspy.Polygon(clad_pts, **self.clad_spec)

        angle=0
        if self.direction=="EAST":
            self.input_port_bot = (self.port[0], self.port[1]-self.wg_sep)
            self.output_port_top = (self.port[0]+2*self.taper_length+self.length, self.port[1])
            self.output_port_bot = (self.port[0]+2*self.taper_length+self.length, self.port[1]-self.wg_sep)
        elif self.direction=="NORTH":
            self.input_port_bot = (self.port[0]+self.wg_sep, self.port[1])
